...
public class FIDO2Applet extends Applet implements ExtendedLength {
    ...
    private void makeCredential(APDU apdu, short lc, short readIdx) {
        ...
        if (tempBools[IDX_OPTION_RK]) {
            ...
            Util.arrayCopy(privateScratch, (short) 0, residentKeyData, (short) (targetRKSlot * CREDENTIAL_ID_LEN), CREDENTIAL_ID_LEN);
            residentKeyUserIdLengths[targetRKSlot] = (byte) userIdLen;
            symmetricWrapper.doFinal(scratch, scratchUserIdOffset, MAX_USER_ID_LENGTH, residentKeyUserIds, (short) (targetRKSlot * MAX_USER_ID_LENGTH));
            residentKeyRPIdLengths[targetRKSlot] = (byte) rpIdLen;
            symmetricWrapper.doFinal(scratch, scratchRpIdOffset, MAX_RESIDENT_RP_ID_LENGTH, residentKeyRPIds, (short) (targetRKSlot * MAX_RESIDENT_RP_ID_LENGTH));
            symmetricWrapper.doFinal(scratch, (short)(scratchPublicKeyOffset + 1), (short)(KEY_POINT_LENGTH * 2), residentKeyPublicKeys, (short) (targetRKSlot * KEY_POINT_LENGTH * 2));
    ...
    private void getAssertion(APDU apdu, short lc, short readIdx) {
        if (Constants.DEBUG) System.out.println("getAssertion - 1");
        if (lc == 0) {
            sendErrorByte(apdu, FIDOConstants.CTAP2_ERR_MISSING_PARAMETER);
        }
        ...
        if (Constants.DEBUG) System.out.println("getAssertion - 2");
        short rpIdLen = 0;
        ...
        if (Constants.DEBUG) System.out.println("getAssertion - 3");
        ...
        short rkMatch = -1;
        if (Constants.DEBUG) System.out.println("getAssertion - 4");
        short paramsRead = 2;
        if (bufferMem[readIdx] == 0x03) { // allowList
            if (Constants.DEBUG) System.out.println("getAssertion - 5");
            readIdx++;
            paramsRead++;
            if (readIdx >= lc) {
                sendErrorByte(apdu, FIDOConstants.CTAP2_ERR_INVALID_CBOR);
            ...
            if (Constants.DEBUG) System.out.println("getAssertion - 6");
            ...
            for (short i = 0; i < numPubKeysToCheck; i++) {
                ...
                if (startOfMatchingPubKeyCredData == (short) -1) {
                    boolean matches = checkCredential(bufferMem, pubKeyIdx, pubKeyLen, scratch, scratchRPIDHashIdx, privateScratch, (short) 0);
                    if (matches) {
        ...
        defaultOptions();
        boolean pinAuthSuccess = false;
        if (Constants.DEBUG) System.out.println("getAssertion - 7");
        // Consume any remaining parameters
        ...
        if (Constants.DEBUG) System.out.println("getAssertion - 8");
        ...
        if (Constants.DEBUG) System.out.println("getAssertion - 9");
        // ---------------
        rkMatch = 0; // TO DO
        startOfMatchingPubKeyCredData = (short)0;
        // ---------------
        ...
        if (Constants.DEBUG) System.out.println("getAssertion - 10");
        ...
        if (Constants.DEBUG) System.out.println("getAssertion - 11");
        ...
        if (rkMatch > -1) {
            ...
            outputLen = Util.arrayCopyNonAtomic(SINGLE_ID_MAP_PREAMBLE, (short) 0, bufferMem, outputLen, (short) SINGLE_ID_MAP_PREAMBLE.length);
        ...
        if (Constants.DEBUG) System.out.println("getAssertion - 12");
        ...
        if (Constants.DEBUG) System.out.println("getAssertion - 13");
        ...
        if (Constants.DEBUG) System.out.println("getAssertion - 14");
        // signature
        ...
        short sigLength = attester.sign(scratch, scratchClientDataHashIdx, clientDataHashLen, bufferMem, (short) (outputLen + 1));
        if (sigLength > 256 || sigLength < 24) {
            throwException(ISO7816.SW_UNKNOWN);
        }
        bufferMem[outputLen] = (byte) sigLength;
        ...
        if (Constants.DEBUG) System.out.println("getAssertion - 15");
        scratchRelease(totalScratchAlloc);
        short rkUIDOffset = scratchAlloc((short)(MAX_USER_ID_LENGTH + 1));

        if (Constants.DEBUG) System.out.println("getAssertion - 16");
        if (rkMatch > -1) {
            if (Constants.DEBUG) System.out.println("getAssertion - 17");
            // Pack the user ID from the resident key into a buffer, after decrypting it
            symmetricUnwrapper.doFinal(residentKeyUserIds, (short) (rkMatch * MAX_USER_ID_LENGTH), MAX_USER_ID_LENGTH, scratch, rkUIDOffset);
            ...
            outputLen = Util.arrayCopyNonAtomic(scratch, rkUIDOffset, bufferMem, outputLen, uidLen);
        }
        if (Constants.DEBUG) System.out.println("getAssertion - 18");

        doSendResponse(apdu, outputLen);
        if (Constants.DEBUG) System.out.println("getAssertion - 19");
    }
